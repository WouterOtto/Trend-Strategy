...
Purpose:
    Receives the sized position list from Script 8 and computes precise,
...
Logic Flow Per Position:
    ┌─────────────────────────────────────────────────────────────────┐
    │ is_new_entry == True?                                           │
    │   YES → calculate_initial_stop(entry_price, entry_ATR)         │
    │         stop_type = "initial"                                   │
    │                                                                 │
    │   NO  → load original entry_price from portfolio_state         │
    │         load current close & ATR from indicators               │
    │         profit_pct = (current_close - orig_entry) / orig_entry │
    │                                                                 │
    │         profit_pct >= 15%?                                      │
    │           YES → trailing active                                 │
    │                 is_friday?                                      │
    │                   YES → recalculate, ratchet up if higher       │
    │                   NO  → carry forward existing trailing stop    │
    │                                                                 │
    │           NO  → retain initial stop from portfolio_state        │
    └─────────────────────────────────────────────────────────────────┘
...
Dependencies (run before this script):
    ...
    07_rank_momentum.py
    08_calculate_position_sizes.py   ← THIS SCRIPT'S PRIMARY INPUT

Inputs:
    - data_cache/portfolio/position_sizes.json   (from Script 8, required)
...
Execution:
    ...
    python scripts/09_calculate_stops.py --as-of-date 2026-01-31 --dry-run

    # Force Friday update even on non-Friday (manual override, use with care)
    python scripts/09_calculate_stops.py --as-of-date 2026-01-31 --force-friday
...
def load_position_sizes(as_of_date: str) -> Tuple[Dict, Dict]:
    """
    Load the sized position list produced by Script 8.

    Expected file: data_cache/portfolio/position_sizes.json

    Returns:
        Tuple of (positions_dict, metadata_dict).
        positions_dict is keyed by symbol.

    Raises:
        SystemExit on missing file, empty positions, or format errors.
    """
    sizes_file = PORTFOLIO_DIR / "position_sizes.json"

    if not sizes_file.exists():
        logger.error(
            f"position_sizes.json not found at {sizes_file}.\n"
            "Please run Script 8 (08_calculate_position_sizes.py) first."
        )
        sys.exit(1)
    ...
    if not positions:
        logger.error(
            "position_sizes.json contains no positions. "
            "Check that Script 8 completed successfully."
        )
        sys.exit(1)

    # Date mismatch warning (non-fatal)
    file_date = metadata.get("as_of_date", "")
    if file_date and file_date != as_of_date:
        logger.warning(
            f"position_sizes.json was generated for {file_date}, "
            f"but --as-of-date is {as_of_date}. "
            "Consider re-running Script 8 for the correct date."
        )

    logger.info(
        f"✔ Loaded {len(positions)} sized positions from {sizes_file}"
    )
...
def load_portfolio_state(path: Optional[Path] = None) -> Dict:
    ...
    logger.info(
        f"✔ Loaded {len(positions)} existing positions from {state_file}"
    )
    return positions
...
def calculate_trailing_stop(
    ...
    Example (Architecture §2.7):
        Entry = €100, Current_Price = €120 (+20%), ATR = €3.00
        Existing_Trailing_Stop = €110

        New_Trailing_Stop = max(110, 120 − 4.0 × 3.00)
                          = max(110, 108)
                          = €110  (unchanged – ratchet prevents step-down)

        Next Friday: Price = €125, ATR = €3.20
        New_Trailing_Stop = max(110, 125 − 4.0 × 3.20)
                          = max(110, 112.20)
                          = €112.20  (ratcheted up, locking in more profit)
...
def process_position_stop(
    ...
    Args:
        ...
        position:       Position dict from position_sizes.json (Script 8 output).
    ...
    if orig_initial_stop is None:
        ...
        if entry_atr_pct > 0:
            ...
            logger.info(
                ...
                f"€{orig_initial_stop:.4f} (not stored in portfolio_state)."
            )
        else:
            logger.warning(
    ...
    if ind_df is not None and not ind_df.empty:
        ...
        current_close = float(latest.get("close", position["close_price"]))
    ...
    if not trailing_check.get("active", False):
        ...
        return {
            ...
            "calculation_note":          (
                ...
                f"Trailing activates at €{trailing_check.get('activation_price', '?'):.4f} "
    ...
    if update_day:
        ...
        calc_note       = (
            ...
            f"€{new_trail_stop:.4f} on update day."
        )
        ...
        calc_note       = (
            f"Non-update day. Trailing stop carried forward at €{new_trail_stop:.4f}. "
...
def calculate_all_stops(
    ...
    Args:
        positions:       Dict of sized positions keyed by symbol (Script 8).
    ...
    for symbol, position in positions.items():
        ...
        moved_flag    = "↑" if stop_record.get("moved_up") else " "

        if stop_price is not None:
            logger.info(
                f"  {symbol:<16} "
                f"stop=€{stop_price:>9.4f}  "
...
def save_stop_levels(stops: Dict, summary: Dict, output_file: Path) -> None:
    """
    Save stop levels to JSON in the format expected by Script 10 and Script 11.
    ...
    logger.info(f"✔ Saved {len(stops)} stop levels → {output_file}")
...
def save_stop_summary(summary: Dict, output_file: Path) -> None:
    ...
    logger.info(f"✔ Saved stop summary → {output_file}")
...
def print_stop_summary(stops: Dict, summary: Dict) -> None:
    ...
    header = (
        f"{'Symbol':<16} {'Stop €':>10}  {'Type':<10} "
        f"{'Initial €':>10}  {'Trailing €':>11}  "
        f"{'P&L%':>7}  {'Dist%':>6}  {'↑':>2}  {'New':>4}"
    )
    logger.info(header)
    logger.info("-" * 100)
    ...
    for symbol, rec in sorted(stops.items()):
        ...
        up_flag   = "↑" if rec.get("moved_up") else " "
        new_flag  = "NEW" if rec.get("is_new_entry") else "hold"

        sp_str    = f"€{sp:>9.4f}" if sp is not None else f"{'ERROR':>10}"
        init_str  = f"€{init_s:>9.4f}" if init_s is not None else f"{'N/A':>10}"
        trail_str = f"€{trail_s:>10.4f}" if trail_s is not None else f"{'—':>11}"
    ...
    logger.info(f"  Trailing updated (↑)  : {summary['trailing_stops_updated']}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Stop-Loss Calculator – Script 9 (v3.2)",
...
Examples:
    ...
    python scripts/09_calculate_stops.py --as-of-date 2026-01-30

    # Dry run: compute and display only – no files written
    python scripts/09_calculate_stops.py --as-of-date 2026-01-31 --dry-run

    # Force trailing stop update on any day (manual override – use with care)
    python scripts/09_calculate_stops.py --as-of-date 2026-01-29 --force-friday
...
Dependencies (run in order):
    01  02  03  04  05  06  07  08_calculate_position_sizes.py  ← then this script
        """
    )
...
def main() -> int:
    ...
    logger.info("=" * 70)
    logger.info("STOP-LOSS CALCULATOR – Script 9")
    ...
    # ── Determine if this is a trailing-stop update day ────────────────────
    ...
    if args.force_friday and not today_is_friday:
        logger.warning(
            f"⚠  --force-friday active: {args.as_of_date} is NOT a Friday. "
    ...
    # ── Load inputs ────────────────────────────────────────────────────────
    try:
        positions, pos_metadata = load_position_sizes(args.as_of_date)
    except SystemExit:
        return 1
    ...
    # ── Save outputs ───────────────────────────────────────────────────────
    if args.dry_run:
        logger.info("\n⚠  Dry-run mode – output files NOT written")
    else:
    ...
    if summary["error_count"] > 0:
        logger.warning(
            f"  ⚠ {summary['error_count']} stop(s) could not be calculated: "
            f"{', '.join(summary['errors'])}"
        )
    if not args.dry_run:
        logger.info("Outputs:")
    ...
    # Return non-zero exit code if any stop failed (alerting for automation)
